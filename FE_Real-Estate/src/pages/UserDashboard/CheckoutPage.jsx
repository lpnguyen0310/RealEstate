      ...
      >
            ...
            <div className="flex flex-col divide-y divide-[#f2f5fb]">
              {[
                ...
                >
                  ...
                  {m.logo && <img src={m.logo} alt="" className="h-5 object-contain opacity-80" />}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="col-span-12 lg:col-auto self-start">
          <div className="rounded-2xl border border-[#e5ecfb] bg-white/80 shadow-md backdrop-blur p-6">
            <div className="flex items-center gap-2 mb-5">
              <div className="h-8 w-8 rounded-lg bg-[#0f2f63] text-white flex items-center justify-center text-lg">💳</div>
              <h2 className="text-[20px] font-semibold text-[#0f2f63]">
                {method === "atm"    && "Thanh toán bằng Thẻ ATM (Stripe Payment Element)"}
                {method === "credit" && "Thanh toán bằng Thẻ tín dụng / ghi nợ (Stripe)"}
            ...
            {(method === "atm" || method === "credit") ? (
              orderId === "N/A" ? (
                <div className="text-[#d43f3a]">Thiếu orderId trên URL. Ví dụ: <code>?orderId=123&amp;amount=99000</code></div>
              ) : (
                <StripeCheckout orderId={orderId} amount={amount} />
              )
            ) : (
              <div className="p-6 flex flex-col items-center justify-center text-[#637089]">
                {method === "wallet" && (<><img src="/images/momo.svg" className="h-10 mb-2" /><p>Thanh toán bằng ví MoMo/ZaloPay (Mock)</p></>)}
                {method === "bank"   && (<><img src="/images/vietqr.svg" className="h-12 mb-2" /><p>Quét mã QR hoặc chuyển khoản thủ công</p></>)}
                {method === "qrpay"  && (<><img src="/images/vnpay.svg" className="h-12 mb-2" /><p>Quét mã VNPAY QR để hoàn tất thanh toán</p></>)}
              </div>
            )}

            <div className="mt-6 flex items-center gap-2 text-[12px] text-[#6b7a90]">
              <span className="inline-flex h-5 w-5 items-center justify-center rounded bg-[#eef4ff]">🔒</span>
        ...
        <div className="col-span-12 lg:col-auto lg:sticky lg:top-0 self-start">
          <div className="rounded-2xl border border-[#e5ecfb] bg-[#fefefe] shadow-md p-5">
            <h3 className="text-[17px] font-semibold text-[#0f2f63] mb-3">Thông tin đơn hàng</h3>
            ...
            <div className="mt-5 rounded-xl bg-[#fff5f5] border border-[#ffd8d8] p-3 text-[#d43f3a] text-sm">
              ⚠️ Vui lòng không đóng cửa sổ này trong quá trình xác thực.
            </div>
