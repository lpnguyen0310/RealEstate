...
import { Tag, Tooltip, Button, Typography } from "antd"; // Chỉ import Button, Typography (nếu cần)
...
// ⭐️ BƯỚC 1: IMPORT COMPONENT MODAL MỚI
import FavoriteUsersModal from "./FavoriteUsersModal"; // Giả sử cùng thư mục

const { Text } = Typography; // Giữ lại nếu cần Text

// === Component Box (giữ nguyên) ===
...
// === Component PostCard (đã rút gọn) ===
export default function PostCard({ post }) {
  // ... (Phần code images, viewer... giữ nguyên)
  ...
  useEffect(() => {
    ...
    if (viewerRef.current) {
      try { viewerRef.current.destroy(); } catch (e) { console.error("Error destroying viewer:", e); }
    ...
    if (imgWrapRef.current) {
        ...
        } catch (e) { console.error("Error initializing viewer:", e); }
    }
    // Cleanup function
    return () => {
      if (viewerRef.current) {
        try { viewerRef.current.destroy(); } catch (e) { console.error("Error destroying viewer on cleanup:", e); }
        viewerRef.current = null;
      }
    };
  }, [images]);
  ...
  return (
    <> {/* Bọc trong Fragment */}
          ...
          <div className="col-span-12 md:col-span-4">
             <div className="rounded-2xl overflow-hidden relative h-full">
              <Swiper modules={[Navigation, SwiperPagination]} navigation pagination={{ clickable: true }} className="!rounded-2xl h-full" >
                {images.map((src, i) => (
                  <SwiperSlide key={i}>
                    <img src={src} alt={`Property image ${i + 1}`} className="h-[240px] w-full object-cover cursor-zoom-in" onClick={() => openViewerAt(i)} onError={(e) => { e.target.onerror = null; e.target.src="https://placehold.co/600x400/EEE/31343C?text=Image+Not+Found"; }} />
                  </SwiperSlide>
                ))}
              </Swiper>
              <button title="Fullscreen" aria-label="View images fullscreen" className="absolute top-3 right-3 h-9 w-9 rounded-full bg-white/90 flex items-center justify-center shadow hover:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500" onClick={() => openViewerAt(0)} >
          ...
          <div className="col-span-12 md:col-span-4 flex flex-col gap-3">
            ...
            <Box>
              <div className="grid grid-cols-2 gap-y-3 text-[#506285]">
                {/* Tình trạng */}
                <div className="flex items-center gap-2"> <span className="text-[#6a84f5]" aria-hidden="true">🗂️</span> <span>Tình trạng tin đăng</span> </div>
                <div className="text-right"> <Tag className="!m-0 !px-3 !py-1 !rounded-lg bg-[#e9f0ff] text-[#3c57d6] border-none"> {post.statusTag || "Nháp"} </Tag> </div>
                {/* Ngày tạo */}
                <div className="flex items-center gap-2"> <span className="text-[#6a84f5]" aria-hidden="true">🕒</span> <span>Ngày tạo</span> </div>
                <div className="text-right">{post.createdAt || "-"}</div>
                {/* Lượt xem */}
                <div className="flex items-center gap-2"> <span className="text-[#6a84f5]" aria-hidden="true">👁</span> <span>Số lượt xem</span> </div>
                <div className="text-right">{post.views ?? 0}</div>
                {/* Lượt yêu thích */}
                <div className="flex items-center gap-2"> <span className="text-[#6a84f5]" aria-hidden="true">❤️</span> <span>Số lượt yêu thích</span> </div>
                <Tooltip title={post.favoriteCount > 0 ? "Xem danh sách người yêu thích" : ""}>
                  <button type="button" className={`text-right font-medium focus:outline-none ${post.favoriteCount > 0 ? 'text-[#ff4d4f] cursor-pointer hover:underline focus:underline' : 'text-[#506285] cursor-default'}`} onClick={handleShowFavorites} disabled={!post.favoriteCount || post.favoriteCount === 0} aria-label={`Xem ${post.favoriteCount} lượt yêu thích`} > {post.favoriteCount ?? 0} </button>
                </Tooltip>
              </div>
            </Box>
          </div>
             ...
             <Box className="h-full w-full">
              <div className="space-y-4 text-[#334e7d]">
                ...
                <div className="leading-6">
                  <div className="flex items-start gap-2"> <span className="text-[#6a84f5] mt-1" aria-hidden="true">📍</span> <div className="font-medium">{post.addressMain || "Chưa cập nhật địa chỉ"}</div> </div>
                  {post.description && ( <div className="flex items-start gap-2 mt-1 text-[#5b6f92]"> <span className="w-4 flex-shrink-0" aria-hidden="true" /> <p className="text-[15px] leading-6 line-clamp-2"> {post.description} </p> </div> )}
                </div>
                {/* Thông số cơ bản */}
      ...
      {/* ⭐️ BƯỚC 2: SỬ DỤNG COMPONENT MODAL ĐÃ IMPORT ⭐️ */}
      <FavoriteUsersModal
        isOpen={isModalOpen}
